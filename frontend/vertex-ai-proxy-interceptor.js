...
(function() {
  ...
  window.WebSocket = function(url, protocols) {
    ...
    if (inputUrl && isValidUrl(inputUrl)) {
      ...
      const host = window.location.protocol === 'file:' ? '127.0.0.1:5000' : window.location.host;
      ...
      try {
        ...
        const proxyFetchOptions = {
          ...
          headers: {
            ...
            'X-App-Proxy': 'kIvwc63tdGn97J8KvdcPzv-UZc5Mb1qu',
          },
          body: JSON.stringify(requestDetails),
        };

        const backendBase = window.location.protocol === 'file:' ? 'http://127.0.0.1:5000' : '';
        console.log('[Vertex AI Proxy Shim] Fetching from local Node.js backend: /api-proxy');
        const proxyResponse = await fetch(`${backendBase}/api-proxy`, proxyFetchOptions);
        ...
        return new Response(JSON.stringify({
            ...
            details: 'Could not connect to the backend server. Please verify if the backend server is running (run "npm run dev-backend" in your project directory). Original error: ' + error.message,
            name: error.name,
            proxiedUrl: inputUrl
          }),
          {
